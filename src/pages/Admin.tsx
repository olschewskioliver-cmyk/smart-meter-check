...
const ROLE_COLORS: Record<string, string> = {
  ...
  office: "bg-office-accent/20 text-office-accent",
  electrician: "bg-success/20 text-success",
};
...
export default function Admin() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-office text-office-fg">
      {/* Top bar */}
      <header className="flex items-center justify-between border-b border-office bg-office-panel px-6 py-3">
        <Wordmark variant="light" subtitle="Administration" />
        <div className="flex items-center gap-3">
          <span className="text-sm text-office-muted">{profile?.full_name}</span>
          <button
            type="button"
            onClick={signOut}
            className="flex items-center gap-1.5 rounded-lg border border-office px-3 py-1.5 text-xs text-office-muted hover:border-office-accent/50 hover:text-office-fg"
          >
                      ...
                      {u.id !== profile?.id && (
                        u.is_active ? (
                          <button
                            ...
                            className="rounded p-1.5 text-office-muted hover:bg-destructive/15 hover:text-destructive"
                          >
                          ...
                          <button
                            ...
                            className="rounded p-1.5 text-office-muted hover:bg-success/15 hover:text-success"
                          >
          ...
          <div className="w-full max-w-md rounded-2xl border border-office bg-office-panel p-6 shadow-2xl">
            ...
            <form onSubmit={handleCreate} className="space-y-3">
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setShowCreate(false)} className="rounded-lg border border-office px-4 py-2 text-sm text-office-muted hover:border-office-accent/50">
                  Abbrechen
                </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-sm rounded-2xl border border-office bg-office-panel p-6 shadow-2xl">
            ...
            <form onSubmit={handleResetPassword} className="space-y-3">
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setResetTarget(null)} className="rounded-lg border border-office px-4 py-2 text-sm text-office-muted hover:border-office-accent/50">
                  Abbrechen
                </button>
